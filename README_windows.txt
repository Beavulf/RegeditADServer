...
## Примечания
- Для production рекомендуется использовать reverse proxy (например, nginx) и HTTPS.
- Если используется SQL Server, убедитесь, что он доступен и параметры подключения указаны в `.env`.
- Если потребуется изменить переменные окружения, перезапустите службу через NSSM или "Службы". 